...
/** Runs the Phase 5 multi-tenant repair as small idempotent steps. */
...
export async function getDb(): Promise<Database> {
  ...
  await markSchemaMigration(db, "20260703000001_membership_display_role_repair");
  await ensureMembershipDeletedAtColumn(db);
  await markSchemaMigration(db, "20260720000001_membership_deleted_at_repair");
  await ensureOutboxUserIdColumn(db);
  await markSchemaMigration(db, "20260720000003_outbox_user_id_repair");
  ...
  await markSchemaMigration(db, "20260715000002_outbox_activity_log_repair");
